...
Durable task JSON now uses locale-independent UTF-16 code-unit key ordering. Existing task ledgers, tombstones, and caches remain structurally readable with no operator action; new appends and rewrites simply use the deterministic ordering going forward. Relay durability is stricter: on first access, any parsed `relay-state.json` record with exact numeric `version: 1` is discarded and replaced with an empty deterministic version-2 state without legacy schema or digest validation. Legacy relay registrations, mailbox records, peer routes, outbox items, and acceptance IDs are discarded. Invalid JSON, a malformed version-2 state, or an unsupported state version fails closed and is not reset.

Task-relay v2 transport records default to 24-hour retention. Set `WOLFPACK_TASK_RELAY_RETENTION_MS` to a positive integer number of milliseconds to change that window. Cleanup runs at gateway initialization and hourly: mailbox rows expire from acknowledgment time or, while unacknowledged, acceptance time; outbox rows expire from forwarding/exhaustion time or, while pending, queue time. Registrations remain while referenced by retained transport work or until the same window has elapsed after lease expiry. Durable per-endpoint cursor watermarks preserve inbox continuity after payload rows are removed. Peer routes are relay-wide and are not pruned by this retention pass.
