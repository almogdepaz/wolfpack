...
For `POST /api/session-open`, the server derives the child harness from the active parent's structured `agentKind`; the client cannot override the harness or command. A client may request a child name, but the server validates it and allocates the canonical name, adding a bounded numeric suffix on collisions. `POST /api/session-create` creates top-level sessions and likewise keeps naming and stable broker identity server-owned. Both operations accept exactly one existing-project selector: `project` for a direct child under `WOLFPACK_DEV_DIR`, or absolute `projectDir` for an existing directory elsewhere. The stored project path is always the server-canonicalized path. These operations follow the ordinary global API auth policy when configured and add no inter-session authorization layer. Tailnet/global Wolfpack access remains the trust boundary.
