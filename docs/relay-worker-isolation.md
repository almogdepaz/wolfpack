...
- Argument ownership is captured before waiting. Envelope shape/JSON admissibility is checked before cloning (which would otherwise strip custom prototypes); the worker repeats the existing validation and retains protocol/lease/caller authority. No cross-call parsed-payload cache is introduced.
...
These limits bound submitted buffers and concurrent calls, **not total RSS or retained file size**. Each active store operation still parses/rewrites history. A worker does not protect the server against process-wide memory exhaustion.
