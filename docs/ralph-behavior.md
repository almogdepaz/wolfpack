...
Default Ralph srt does not allow arbitrary local listener creation. Plans or verification steps that need Unix domain socket bind/listen (for example, starting `wolfpack-broker` inside srt), localhost TCP bind/listen, browser/dev servers, or broker-backed perf/integration tests that spawn their own broker must run that phase outside srt (`--sandbox false`) or use an explicitly requested socket-capable srt profile. The default settings allow sandboxed clients to connect to Wolfpack's configured host broker socket path where macOS srt supports path-scoped Unix sockets, but they do not enable `allowAllUnixSockets` and they do not make broker startup inside default srt supported. If a test truly needs Unix socket bind/listen inside srt, the profile must include `network.allowUnixSockets` for the socket path/dir and `filesystem.allowWrite` for the socket directory.
