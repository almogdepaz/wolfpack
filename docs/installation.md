...
On later runs, the installer validates and compares the selected server/broker pair before changing services or managed binaries. If the pair differs and its broker service is running, replacement ends broker-owned sessions: interactive runs require confirmation through the controlling terminal; unattended runs require `WOLFPACK_INSTALL_ALLOW_SESSION_LOSS=1`. When managed services existed, replacement stops server then broker, removes descriptors, runs deferred setup, then recreates and starts broker then server.
...
The launcher leaves the package store unchanged: it requires an exact package name and version plus both regular, owner-executable payloads, then executes the server directly. Mutating package commands reconcile a changed pair to `~/.wolfpack/bin` only after validation, any required broker-session confirmation, and verified service shutdown; matching bytes alone do not restart services, while changed setup settings can refresh or restart the server only. If package-pair activation fails, rerun the exact version-pinned Bunx or npx command printed for the runner that launched it. Missing, mismatched, incomplete, and non-executable pairs fail clearly; it never falls back to a local binary. macOS server signing belongs to the release build, not to package execution, and is not publisher identity verification.
...
5. after both checks pass, compares the validated pair with the managed pair; a differing pair stops server then broker, removes only Wolfpack's managed descriptors and binary paths, installs both files with mode `0755`, and recreates/starts broker then server when managed services existed; and
6. leaves unrelated `wolfpack` commands or symlinks unchanged, attempts its managed PATH symlink, and runs the exact managed binary's setup. A matching pair causes no service stop or descriptor recreation solely because its bytes match; changed setup settings can refresh or restart the server only. Activation failure exits nonzero and prints the exact reinstall command for the selected release.

On a clean install, `exec "$MANAGED_BINARY" setup` replaces Bash, so its EXIT trap does not run and a private `.install.*` directory can remain under `~/.wolfpack/bin`. On a configured upgrade, setup returns control to Bash before full broker-then-server service activation, and the EXIT trap removes the staging directory when the installer exits. Any retained clean-install directory contains the public release checksum list and neither moved binary; remove it only after confirming no installer is active.

Direct setup can refresh the installed server descriptor or request an immediate server-only restart when descriptor settings or remote-access policy change. During a configured installer upgrade, restart deferral writes descriptor changes without a second activation after pair replacement. On a clean install without a service, accepting login-service installation starts both the server and broker. While active sessions matter, run the upgrade from an external terminal and verify sessions and service state afterward.
...
The file replacement itself does not create a PATH symlink or perform service activation. This manual path does not implement the installer’s managed replacement lifecycle; rerun the supported installer if an existing service must be reconciled to the validated pair. The moves overwrite the managed server/broker pair and there is no automatic rollback, so stop before them or retain your own backup if inspection fails. While active sessions matter, run these steps from an external terminal and verify sessions and service state afterward. `~/.wolfpack/bin/wolfpack uninstall --yes` removes Wolfpack-managed files while preserving unrelated commands; see [uninstall](#uninstall).
