...
6. leaves unrelated `wolfpack` commands or symlinks unchanged, attempts its managed PATH symlink, and runs the exact managed binary's setup. A matching pair causes no service stop or descriptor recreation. Activation failure exits nonzero and prints the exact reinstall command for the selected release.
