...
| Session Backends | `src/server/backend.ts`, `src/server/pty-backend.ts`, `src/server/tmux-backend.ts`, `src/server/tmux.ts`, `src/server/ring-buffer.ts`, `src/server/mock-backend.ts` | [.context/session-backends.md](.context/session-backends.md) |
...
- **tmux / PTY** — external terminal multiplexer (tmux) or in-process PTY (Bun spawn); session lifecycle managed by backend modules
...
6. **Shell args never interpolated**: all tmux, git, and agent invocations use array form (`execFile`, `execFileSync`, `spawn` with args array). No string shell interpolation. One exception: `tmuxNewSession` uses `shellEscape` before `-lic`. (`src/server/tmux.ts:314`)
